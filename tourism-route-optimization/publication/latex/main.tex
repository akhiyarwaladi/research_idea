...
The contributions of this study are threefold. First, we provide the first multi-algorithm metaheuristic comparison for tourism routing in Yogyakarta that uses real road network distances rather than Euclidean approximations. Second, we quantify the road-to-Euclidean distance deviation across 300 attraction pairs, providing empirical evidence for why straight-line distances are inadequate for practical route planning in this region. Third, we present a parameter sensitivity analysis of MMAS that reveals the interaction structure among its three main control parameters, offering guidance for practitioners tuning ACO algorithms on similar problem instances.
...
Ruiz-Meza and Montoya-Torres \cite{ruizmeza2022} surveyed tourist trip design, covering orienteering formulations with time windows \cite{zhong2023,sylejmani2024}. Sun et al. \cite{sun2022} proposed multi-objective ACO for route recommendation, incorporating travel time and tourist preferences alongside distance. Most studies in tourism routing use Euclidean or haversine distances for simplicity, but this assumption has been questioned by vehicle routing researchers. OSMnx \cite{boeing2025,boeing2022} has made real road network extraction practical for any region worldwide, and Boyaci et al. \cite{boyaci2021} showed that Euclidean approximations introduce 10--30\% errors in European vehicle routing. Tatit et al. \cite{tatit2024} confirmed similar deviations for spatial database queries, noting that the error varies with road network density and terrain.

Within Indonesia, Nasution et al. \cite{nasution2024} applied vehicle routing with time windows to tourism itineraries but relied on Google Maps API calls rather than a pre-computed road network. Fathurrohman et al. \cite{fathurrohman2025} formulated a green orienteering problem for Yogyakarta that considered carbon emissions \cite{thipsingh2022}, but tested only a single algorithm. Neither study provided the multi-algorithm statistical comparison over pre-computed real road distances offered here, nor did they analyse parameter sensitivity or scalability behaviour.
...
Twenty-five attractions in DIY were selected across six categories: cultural heritage sites (Kraton, Taman Sari, Kotagede), historical landmarks (Benteng Vredeburg, Tugu), religious temples (Borobudur, Prambanan, Ratu Boko), nature and recreation areas (Parangtritis, Indrayanti, Goa Pindul, Hutan Pinus Mangunan, Tebing Breksi, HeHa Sky View), museums (Sonobudoyo, Affandi, Ullen Sentalu), and urban attractions (Malioboro, Beringharjo, Taman Pintar, Gembira Loka Zoo, Purawisata, Monumen Jogja Kembali, Alun-Alun Utara, Alun-Alun Kidul). Table~\ref{tab:attractions} lists each attraction with its geographic coordinates.

The road network was downloaded via OSMnx \cite{boeing2025} using the query ``Daerah Istimewa Yogyakarta, Indonesia'' with \texttt{network\_type=drive}, yielding 153,334 nodes and 200,104 edges after undirected conversion. Each attraction was snapped to its nearest network node, and the $25 \times 25$ distance matrix was computed using Dijkstra's algorithm. Fig.~\ref{fig:attraction_map} shows the spatial distribution of these attractions. The study area spans roughly 60~km north-to-south (from Borobudur/Ullen Sentalu in the highlands to Pantai Indrayanti on the coast) and 50~km east-to-west (from Goa Pindul to Borobudur), with a dense cluster of 10 attractions within the urban core of Yogyakarta city.

The resulting distance matrix is visualised in Fig.~\ref{fig:distance_matrix}. The largest pairwise distance is 93.3~km (Borobudur to Pantai Indrayanti), while the smallest is 0.3~km (Alun-Alun Utara to Museum Sonobudoyo). Urban core attractions form a visible low-distance block in the upper-left portion of the matrix. The asymmetry between some pairs reflects the one-way street structure retained during Dijkstra computation before the matrix was symmetrised by taking the minimum of each directed pair.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{fig1_attraction_map.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{fig9_distance_matrix.png}
\caption{Road distance matrix (km) for the 25 tourist attractions. Darker cells indicate longer distances. The urban core attractions (IDs 1--10) form a low-distance cluster in the upper-left block.}
...
\begin{table}[!htbp]
\centering
\caption{Algorithm performance over 30 runs. Best values in bold; NN is deterministic. RPD from $D^{*} = \SI{283.76}{km}$.}
\label{tab:performance}
\footnotesize
\begin{tabular}{@{}lrrrrr@{}}
\toprule
 & Best & Mean & Std & RPD & Time \\
 & (km) & (km) & (km) & (\%) & (s) \\
\midrule
MMAS & 283.76 & 284.35 & 0.85 & 0.21 & 1.704 \\
ACS  & 283.94 & 285.49 & 1.80 & 0.61 & \textbf{0.327} \\
GA   & 291.31 & 302.13 & 6.79 & 6.47 & 0.676 \\
\textbf{SA} & \textbf{283.76} & \textbf{283.82} & \textbf{0.08} & \textbf{0.02} & 3.496 \\
NN   & 297.10 & 297.10 & 0.00 & 4.70 & 0.001 \\
\bottomrule
\end{tabular}
\end{table}

Fig.~\ref{fig:bar_summary} provides a visual comparison of the key metrics from Table~\ref{tab:performance}. The distance panel makes the gap between GA and the other algorithms immediately apparent, while the time panel shows that ACS and GA are the fastest algorithms but for very different reasons: ACS finishes quickly because its pseudo-random rule concentrates search effort, while GA's population-based evaluation is inherently parallel.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{fig10_bar_summary.png}
\caption{Summary of algorithm performance. Left: best, mean, and worst distances; right: mean computation time per run.}
\label{fig:bar_summary}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{fig4_boxplot.png}
...
\begin{table}[!htbp]
\centering
\caption{Wilcoxon signed-rank test results (significance level 0.05). All six pairwise comparisons are significant.}
\label{tab:wilcoxon}
\footnotesize
\begin{tabular}{@{}lrl@{}}
\toprule
...
GA converged the most slowly. Its initial solutions were substantially longer because it starts from purely random permutations without a nearest-neighbour seed. GA was still improving at iteration~500, suggesting the budget was insufficient and that hybridisation with local search operators could narrow the gap. The convergence profiles explain why the standard deviations in Table~\ref{tab:performance} differ so widely: SA's early convergence leaves little room for run-to-run variation, while GA's incomplete convergence means each run may stop at a different point along its descent curve. MMAS and ACS fall in between, with MMAS achieving lower variance thanks to its longer convergence window.
...
The ACS route followed a nearly identical sequence, differing by only one swap in the urban core and adding less than \SI{0.2}{km}. GA's best tour contained visible detours where the route jumped between non-adjacent zones instead of completing one cluster before moving to the next. This is a typical weakness of OX crossover, which preserves relative ordering but does not directly optimise edge connections.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{fig6_route_comparison.png}
\caption{Best routes found by each algorithm on the road network. SA and MMAS share the same optimal tour structure (\SI{283.76}{km}); ACS differs by one swap (\SI{283.94}{km}); GA shows visible detours (\SI{291.31}{km}).}
\label{fig:routes}
\end{figure}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{fig5_parameter_sensitivity.png}
\caption{MMAS parameter sensitivity heatmaps. Each cell shows the mean distance (km) over 10 runs. The best configuration (pheromone weight 2.0, heuristic weight 2, evaporation rate 0.02) matches SA's mean of \SI{283.82}{km}.}
...
The bottom panel of Fig.~\ref{fig:scalability} shows runtime. MMAS and ACS grew roughly quadratically with problem size, consistent with the $O(m \cdot n^2)$ cost of their pheromone update mechanism, where $m = n$ ants each evaluate $n$ candidate cities at each step. GA scaled linearly because its population size and number of generations were held constant. SA's runtime stayed nearly constant at about \SI{3.5}{s} regardless of problem size, since the number of temperature steps is fixed by the cooling schedule, and each 2-opt evaluation is $O(1)$ using the incremental cost formula in Eq.~\ref{eq:2opt_delta}. This constant-time property makes SA attractive for larger tourism networks where computation budget is limited, though the quality advantage may diminish as problem size grows beyond the 25-node range tested here.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{fig8_scalability.png}
\caption{Scalability analysis for 10, 12, 15, 20, and 25 attractions. Top: mean distance; bottom: mean computation time. GA diverges as problem size increases; SA runtime remains constant.}
...
Fig.~\ref{fig:euclidean_road} compares the two distance measures across all 300 attraction pairs. The scatter plot shows a strong linear correlation (R-squared = 0.97), but with a consistent positive bias: road distances averaged 18\% longer (mean ratio 1.18, standard deviation 0.12). The ratio was lowest (around 1.05) for pairs connected by direct highway segments, such as the Yogyakarta--Prambanan corridor along the main east-west highway. The highest ratios (up to 1.72) occurred for pairs separated by rivers, volcanic slopes, or dense urban fabric where roads must follow indirect paths. For example, the Euclidean distance from Candi Borobudur to Pantai Indrayanti is 48~km, but the road distance is 83~km because vehicles must descend the Menoreh hills, cross through the Yogyakarta urban area, and navigate the winding Gunung Kidul karst roads to reach the coast.

Because the bias is consistent, the errors compound across a multi-stop tour. For the 25-attraction problem, relying on Euclidean distances would underestimate total tour length by roughly \SI{40}{--}\SI{50}{km}, enough to make a planned day trip infeasible. More critically, Euclidean-based optimization may produce a different tour order than road-based optimization, because the relative ranking of edge costs changes when detour ratios vary across the network. Two attractions that appear close in Euclidean space may be far apart on the road network if a river or mountain range lies between them.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{fig7_euclidean_vs_road.png}
...
\textbf{Why SA outperforms ACO on this instance.} The 25-attraction problem has a particular structure that favours SA with 2-opt. Most of the attractions cluster geographically into four or five groups (the urban core, the eastern temples, the southern coast, the highlands, and Borobudur). A 2-opt move can efficiently remove crossing edges between these clusters, producing large distance improvements in a single step. ACO variants, by contrast, build tours node by node and rely on pheromone accumulation over many iterations to discover the same cluster structure. For this moderate problem size, the direct edge-swap mechanism of 2-opt converges faster than pheromone-guided construction. On larger or less clustered instances, the balance might shift in favour of ACO, which can explore a wider set of tour structures through its population of ants.

\textbf{Practical implications.} The optimal \SI{283.76}{km} tour reported in Table~\ref{tab:performance} translates to roughly 7 hours of driving at Yogyakarta's typical mixed-road speed of \SI{40}{km/h}. Combined with 30--60 minutes spent at each attraction, visiting all 25 sites listed in Table~\ref{tab:attractions} would span 3--4 days. The geographic clustering visible in the optimal route (Fig.~\ref{fig:routes}) suggests a natural breakdown into daily itineraries: (1)~the urban core (Kraton, Malioboro, Taman Sari, and surrounding sites), (2)~the eastern temple complex (Prambanan, Ratu Boko, Tebing Breksi) combined with the northern highland (Ullen Sentalu), (3)~the southern coast (Parangtritis, Indrayanti, Goa Pindul, Hutan Pinus), and (4)~a western excursion to Borobudur. Tour operators could use these clusters as ready-made day-trip modules.

Looking at the distance matrix in Fig.~\ref{fig:distance_matrix}, the urban core attractions (IDs 1--10) are all within 5~km of each other, making them walkable or reachable by short rides. The coastal attractions (Parangtritis, Indrayanti) are 30--50~km from the urban core, and Goa Pindul is another 40~km east, so the southern loop alone requires substantial driving. Borobudur sits 40~km northwest and is best combined with Ullen Sentalu (25~km northeast of Borobudur) as a full-day excursion from the city.
...
\textbf{Limitations.} This study optimises total distance only. Travellers also care about travel time, entrance fees, opening hours, and personal preferences; multi-objective formulations \cite{ruizmeza2022,sun2022} would address these. The road distances are static and do not account for traffic congestion or time-of-day variations, which can be substantial in Yogyakarta: the Malioboro corridor and the road to Borobudur experience heavy congestion during holidays and weekends. The analysis covers a single destination with 25 attractions; testing on larger instances (50--100 attractions) or other Indonesian cities with different road topologies would test generalisability. The comparison also does not include orienteering formulations with time windows \cite{sylejmani2024,fathurrohman2025}, which would be more realistic for tourists with limited time, or near-exact solvers such as LKH \cite{zheng2023}, which would provide a tighter upper bound on solution quality.
...
Future work should extend this framework in several directions. Multi-objective formulations that include travel time, entrance fees, opening hours, and visitor preferences would produce more practical itineraries. Incorporating dynamic traffic data from sources such as Google Maps or local traffic monitoring systems would improve the accuracy of travel time estimates, particularly during peak tourist seasons when congestion around Borobudur and Malioboro can double travel times. Testing the approach on other Indonesian tourism destinations, such as Bali, Bandung, or Lombok, would help assess generalisability across different road network topologies and attraction distributions.
