...
The ACS route (Fig.~\ref{fig:routes}c) followed a nearly identical sequence, differing by only one swap in the urban core and adding less than \SI{0.2}{km}. GA's best tour (Fig.~\ref{fig:routes}d) contained visible detours where the route jumped between non-adjacent zones instead of completing one cluster before moving to the next---a typical weakness of OX crossover, which preserves relative ordering but does not directly optimise edge connections.
